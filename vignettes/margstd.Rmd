...
We fit the same risk difference model as in the [Get Started vignette](risks.html#adjusted-risk-differences), this time specifying `approach = "margstd_delta"`:
...
Standardization can only be done over one exposure variable, and thus no cofficients are estimated for other variables in the model.
...
When using `tidy()` to access model results, the parameter `bootverbose = TRUE` will add bootstrapping parameters and properties to tibble

```{r margstd_bootverbose}
fit_margstd3 <- riskdiff(formula = death ~ stage + receptor, data = dat, 
